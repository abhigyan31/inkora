...
function Profile() {
  ...
  /* Asking the server for the saved ones directly beats
     fetching every blog and filtering in the browser. */
  const { blogs: savedBlogsList } = useBlogs({ saved: 1 });

  const { isSaved, toggleSave, savedCount } = useSaved();
  const { likeCount } = useLikes();

  /* Every count on this page is worked out by the database. */
  const { stats } = useUserProfile(user?.username);
  ...
  async function handleProfileImage(event) {
    ...
    try {
      ...
      await updateCurrentUser({ avatar: path });
  ...
  async function handleCoverImage(event) {
    ...
    try {
      ...
      await updateCurrentUser({ coverImage: path });
        ...
        <section className="overflow-hidden rounded-[30px] border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          ...
          <div className="relative px-5 pb-7 sm:px-8 sm:pb-9">
            ...
            <div className="mt-7 flex flex-wrap gap-x-8 gap-y-5 border-y border-neutral-100 py-5 dark:border-neutral-800">
              ...
              <ProfileStat
                value={stats?.blogs ?? myBlogs.length}
                label="Blogs"
              />

              <ProfileStat
                value={formatCount(stats?.likesReceived ?? 0)}
              ...
              <ProfileStat
                value={formatCount(stats?.saved ?? savedCount)}
                label="Saved"
              />
            </div>
          </div>
