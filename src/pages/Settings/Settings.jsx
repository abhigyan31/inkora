...
function Settings() {
  ...
  async function handleDeleteAccount() {
    ...
    try {
      /* Everything cascades from the users row, so the blogs,
         comments, likes, follows and notifications go with
         it. The API deletes the uploaded files first, since
         those sit on disk and nothing else would clear them. */
            ...
            {activeSection === "account" && (
              <div>
                ...
                <div className="space-y-6 p-6 sm:p-8">
                  ...
                  <SaveButton onClick={saveChanges} saving={saving} />
                </div>
              </div>
            )}
      ...
      {showDelete && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[28px] bg-white p-7 text-neutral-900 shadow-2xl">
            ...
            <label className="mt-4 block text-xs font-bold text-neutral-700">
              Your password
