...
function FeedCard({
  ...
  /* The API counts these, so the card doesn't have to fetch
     every blog's comments just to show a number. */
  const commentCount = Number(blog.comments) || 0;
...
function Feed() {
  ...
  /* The tab is a query, not a filter. Sorting and the
     following list are the database's job - doing it here
  ...
  const { blogs: visibleBlogs, loading } = useBlogs(query);

  /* Suggested writers come from whoever actually wrote the
     blogs on screen. A hardcoded list would show people who
     aren't in the database, and following them would fail. */
  const { blogs: recentBlogs } = useBlogs({});
  ...
  const writers = useMemo(() => {
    ...
    recentBlogs.forEach((blog) => {
      ...
      if (!seen.has(author.username)) {
        seen.set(author.username, author);
      }
    });
  ...
  async function handleToggleLike(blog) {
    try {
      await toggleLike(blog);
    } catch (error) {
      showToast(error?.message || "Sign in to like blogs", "error");
    }
  }
  ...
  async function handleToggleSave(id) {
    try {
      ...
      showToast(error?.message || "Sign in to save blogs", "error");
    }
  }
  ...
  async function handleToggleFollow(username) {
    try {
      ...
      showToast(error?.message || "Sign in to follow writers", "error");
    }
  }
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleToggleFollow(writer.username)
                          }
