...
import { saveFile, deleteFile, isFileRef } from "../../utils/fileStore";
import { getAuthorBlock, useCurrentUser } from "../../utils/session";
...
function Create() {
  ...
  async function handlePublish() {
    ...
    try {
      ...
      if (thumbnailFile) {
        finalThumbnail = await saveFile(thumbnailFile);

        /* Replace the old file when editing. */
        if (isEditing && thumbnailRef && isFileRef(thumbnailRef)) {
          deleteFile(thumbnailRef);
        }
      }

      if (pdfFile) {
        finalPdf = await saveFile(pdfFile);

        if (isEditing && pdfRef && isFileRef(pdfRef)) {
          deleteFile(pdfRef);
        }
      }
      ...
      setErrors((previous) => ({
        ...
        publish:
          error?.name === "QuotaExceededError"
            ? "Your browser storage is full. Delete an older blog and try again."
            : error?.message ||
      ...
      <div className="flex min-h-screen items-center justify-center bg-neutral-50 px-5 dark:bg-neutral-950">
        <div className="w-full max-w-md rounded-3xl border border-neutral-200 bg-white p-10 text-center shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
          ...
          <p className="mt-3 text-sm leading-6 text-neutral-500 dark:text-neutral-400">
            You can only edit blogs you published on this device. Demo blogs
            cannot be edited.
          </p>
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-[28px] border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
              ...
              <p className="mt-3 text-center text-[10px] leading-4 text-neutral-400">
                Your blog is saved on this device. The PDF and thumbnail go
                into browser file storage, not localStorage.
              </p>
            </div>
          </aside>
        </div>
      ...
      {published && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[30px] bg-white p-8 text-center shadow-2xl dark:bg-neutral-900">
            ...
            <p className="mt-3 text-sm leading-6 text-neutral-500 dark:text-neutral-400">
              "{publishedBlog?.title}" is now at the top of your feed and on
              your profile. The PDF and thumbnail are stored in this browser.
            </p>
